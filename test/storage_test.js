...
describe('Storage', () => {
  ...
  describe('constructor', () => {
    it('should return error when no real storage supplied', () => {
      expect(() => { new Storage() }).to.throw(Error, '`storage` is required')
    })
  })

  describe('hasItem()', () => {
    before(() => { storage.setItem('foo', 'bar') })
    after(() => { storage.removeItem('foo') })
  ...
  describe('getItem()', () => {
    before(() => { storage.storage.setItem('foo', '{"foo":"bar"}') })
    after(() => { storage.removeItem('foo') })
